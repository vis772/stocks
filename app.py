...
:root {
    --bg:#080c10; --bg2:#0e1419; --bgcard:#111820; --border:#1e2d3d; --borderhi:#2a4060;
    --green:#00ff88; --blue:#00aaff; --amber:#ffaa00; --red:#ff3355; --purple:#aa55ff;
    --t1:#e8f0f8; --t2:#7a9ab8; --t3:#3a5068; --tdim:#1e2d3d;
}
...
.stTextInput input, .stTextArea textarea {
    ...
    color: var(--t1) !important; border-radius: 6px !important; font-family: 'JetBrains Mono', monospace !important;
}
.stButton > button {
...
# ══════════════════════════════════════════════════════════════════════════════
# HELPER FUNCTIONS — ALL DEFINED BEFORE ANY UI CODE
# ══════════════════════════════════════════════════════════════════════════════
...
def render_deep_dive(r: dict):
    ...
    if not hist.empty:
        ...
        if entry:
            fig.add_hline(y=entry, line_color="rgba(0,170,255,0.35)", line_dash="dash",
                          annotation_text="Entry", annotation_font_color="#00aaff")
        if stop_:
            fig.add_hline(y=stop_, line_color="rgba(255,51,85,0.35)", line_dash="dash",
                          annotation_text="Stop", annotation_font_color="#ff3355")
        fig.update_layout(
...
with st.sidebar:
    ...
    if st.button("SAVE PORTFOLIO", use_container_width=True):
        ...
        if saved:
            st.success(f"✓ {saved} holding(s) saved")
        for e in errors:
            st.error(e)
...
with tab1:
    ...
    if not valid:
        st.markdown('<div class="empty"><div class="ico">⚡</div><h3>RUN A SCAN TO SEE RESULTS</h3><p style="color:#3a5068;font-family:\'JetBrains Mono\',monospace;font-size:0.83em;max-width:380px;margin:0 auto;">Enter tickers in the sidebar or use the default universe.</p></div>', unsafe_allow_html=True)
    else:
        sc_map = {}
        for r in valid:
            s = r.get("signal","—")
            sc_map[s] = sc_map.get(s, 0) + 1

        labels = [
            ("Strong Buy","Strong Buy Candidate","#00ff88"),
            ("Spec Buy","Speculative Buy","#00dd66"),
            ("Watchlist","Watchlist","#ffdd00"),
            ("Hold","Hold","#ffaa00"),
            ("Trim","Trim","#ff7700"),
            ("Sell","Sell","#ff3355"),
            ("Avoid","Avoid","#cc1133"),
        ...
        with fc2:
            sig_filter = st.multiselect("Signal", ["Strong Buy Candidate","Speculative Buy","Watchlist","Hold","Trim","Sell","Avoid"], default=[], placeholder="All signals")
        with fc3:
            hide_crit = st.checkbox("Hide critical flags", False)
    ...
    if portfolio_df.empty:
        ...
        if len(holdings_analysis) > 1:
            ...
            fig_pie = go.Figure(go.Pie(
                ...
                marker=dict(
                    colors=["#00ff88","#00aaff","#ffaa00","#ff3355","#aa55ff","#00ddff","#ff7700"],
